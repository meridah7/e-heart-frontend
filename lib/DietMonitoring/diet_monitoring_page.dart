    ...
    MultipleChoiceQuestion(
      '你本次要做的记录包括',['饮食记录','食物清除记录'],
      {
        '食物清除记录':[
          ...
          MultipleChoiceQuestion('食物清除的时候感受到的情绪种类', ['伤心', '疲惫', '紧张', '无聊', '兴奋', '羞愧', '愤怒', '恐惧', '平静', '开心','其他'], {
            '其他':[
        ...
        '饮食记录':[
          ...
          SliderQuestion('进食的时候感受到的情绪强度',{}, min: 1, max: 7, divisions: 6, labelBuilder: (value) {
            if (value == 1) return '很不开心';
            else if (value == 4) return '一般';
            else if (value == 7) return '超开心';
            return value.toInt().toString();
          }),
          MultipleChoiceQuestion('情绪种类', ['伤心', '疲惫', '紧张', '无聊', '兴奋', '羞愧', '愤怒', '恐惧', '平静', '开心','其他'], {
            '其他':[
              TextQuestion('请输入其他情绪种类', false),
            ]
          }),
          SingleChoiceQuestion('在哪吃', ['家里', '宿舍', '公司', '食堂', '饭店', '车上','其他'], {
            '其他':[
              TextQuestion('请输入其他地点', false),
            ],
            '家里':[
              SingleChoiceQuestion('是在家里的', ['客厅','卧室','餐桌','厨房','洗手间','其他'], {
                '其他':[
                  TextQuestion('请输入其他地点', false),
                ],
              })
          ...
          SliderQuestion('是否暴食',{'1':[
            SingleChoiceQuestion('请努力识别这次暴食的诱发因素并填写它！', ['不知道','点击填写'],description: 
            '这一部分很重要！如果没有识别出来，请选择未知。\n\n'
            ...
            {
              '点击填写':[
                TextQuestion('questionText', false)
              ]
            })
          ]}, min: 0, max: 10, divisions: 10, labelBuilder: (value) {
            if (value == 0) return '没有暴食';
            else if (value == 5) return '中度暴食';
            else if (value == 10) return '重度暴食';
            return value.toInt().toString();
          }),
          TextQuestion('更多注释（想法，感受等等）', false, description: '记录任何可能会影响你这次饮食的东西，不管是你的纠结，想法还是情绪都可以。努力写一些！这一块的记录往往会在之后成为你改善暴食的奇招！'),
          MultipleChoiceQuestion(
            '也许你有一些困惑：', 
            ...
            {
            '我感觉记录我的饮食尤其是暴食是一件很羞耻的事情':[
                SingleChoiceQuestion('暴食很容易让我们愧疚，因此记下它确实是很不容易的事情。但你只需要坚持过去头几天！你会发现，之后有效分析带来的收获能够帮助你很好的应对羞耻的不快。别忘记，当你感觉到困难的时候，往往是你在面临最关键的改变部分！', ['好的！'], {})
              ], 
            '我极力想要忘记吃东西这件事，但记录自己吃了什么会让我更加专注于吃，这会不会让我更难受？':[
                SingleChoiceQuestion('在饮食日志的前几天，你确实可能发现你对饮食的注意力增加了。但是克服暴食的第一步就是直面它！过了几天，你就会习惯于记录自己的饮食，就像平时吃饭喝水一样简单。', ['好的！'], {})
              ],
            '我曾经记录过自己的饮食，但感觉没什么帮助':[
                SingleChoiceQuestion('还是那句话，越感到困难的时候越说明你在面临改变越关键的部分。试着设置闹钟，或者找个朋友提醒自己～', ['好的！'], {})
              ],
            '我感觉很难坚持':[
                SingleChoiceQuestion('你的记录很有可能不像本方案教你的那么系统。同时，本方案还会基于你的饮食日志，教你如何分析自己的饮食模式。这是你一个人做不到的！', ['好的！'], {})
              ],
