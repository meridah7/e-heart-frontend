...
Future<List<Question>> generateDietPlanReview() async {
  ...
  try {
    ...
    Response missingPlanDaysRes = await dioClient
        .getRequest('/meal_plans/missing-planning-days/$startDate/$endDate');
    // 饮食计划不合理间隔次数
    Response unreasonableIntervalCountRes = await dioClient.getRequest(
        '/meal_plans/unreasonable-interval-counts/$startDate/$endDate');
    if (missingPlanDaysRes.statusCode == 200 &&
      ...
      return [
        ...
        SingleChoiceQuestion(
            ...
            description: '小E看到您已经做了${7 - missingDays}天的计划'),
        ChartQuestion(
            "我的饮食计划中的每餐间隔时间",
            [
        ...
        SingleChoiceQuestion(
            ...
            // TODO 补齐状态
            description: "小E看到您本周有$missingDays天仅仅计划了3餐或3餐以下"),
