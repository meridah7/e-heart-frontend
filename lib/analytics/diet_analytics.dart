        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                _buildStatItem(
                    '总餐次', '${_dietStats['totalMeals']}', Icons.restaurant),
                _buildStatItem(
                    '计划执行率',
                    '${(_dietStats['planCompletionRate'] * 100).toStringAsFixed(1)}%',
                    Icons.assignment_turned_in),
                _buildStatItem(
                    '平均暴食指数',
                    '${_dietStats['avgBingeScore'].toStringAsFixed(1)}',
                    Icons.trending_up),
              ],
            ),
          ],
                      ...
                      sideTitles: SideTitles(
                        ...
                        getTitlesWidget: (value, meta) {
                          ...
                          final weekDay = DateFormat('E').format(
                              DateTime.fromMillisecondsSinceEpoch(_dietTrends[
                                      value.toInt() % _dietTrends.length]
                                  .date));
                    ...
                    LineChartBarData(
                      spots: _dietTrends.asMap().entries.map((entry) {
                        return FlSpot(entry.key.toDouble(),
                            entry.value.bingeScore.toDouble());
                      }).toList(),
                      isCurved: true,
...
class DietTrend {
  ...
  DietTrend(
      {required this.date, required this.bingeScore, required this.mealCount});
